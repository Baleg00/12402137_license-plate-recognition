...
from helpers import bbox_to_mask, extract_plate_crop
from metrics import DiceLoss
from training import bce_dice_loss, iou_score, load_model
...
def test_bce_dice_loss_returns_scalar_and_is_finite() -> None:
    ...
    loss = bce_dice_loss(logits, targets, bce_weight=0.7)
    assert isinstance(loss, torch.Tensor)
    assert loss.ndim == 0
    assert torch.isfinite(loss).item()
...
def test_iou_score_range_and_extremes() -> None:
    ...
    iou = iou_score(logits, targets, thr=0.5)
    ...
    iou2 = iou_score(logits2, targets, thr=0.5)
    assert 0.0 <= iou2 <= 1.0
    assert iou2 < 1e-3
...
def test_extract_plate_crop_returns_expected_height() -> None:
    ...
    crop = extract_plate_crop(img, mask, out_h=32, max_w=256, use_adaptive=False)
